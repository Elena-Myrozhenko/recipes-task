...
export const getVisibleRecipes = (state, filter) => {
	return fromList.getIds(state.listByFilter[filter]).map(id => fromById.getRecipe(state.byId, id)).filter(r => state.mask.startsWith('#') ? r.tags.indexOf(state.mask.replace('#', '')) !== -1 : r.title.includes(state.mask)).sort((a, b) => a.timestamp - b.timestamp).reverse();
}
