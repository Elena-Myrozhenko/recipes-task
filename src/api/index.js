...
export const fetchRecipes = (filter) => 
	delay(250).then(() => {
		if (Math.random() > 0.75) throw new Error('randomly forced throw, 25% chance');
		switch (filter) {
			case 'all':
				return recipes;
			case 'favorites':
...
export const fetchRecipe = (id) => 
	delay(250).then(() => {
		...
		if (!recipe) throw new Error('Fake API creates new ids on refresh, hence your recipe isn\'t found. But we have a copy in localStorage, so we\'re good.');
		return recipe;
	});
...
export const addRecipe = (id, rawRecipe) =>
	delay(250).then(() => {
		if (!id) {
			...
			if (rx !== -1) {
				recipes[rx] = { ...recipes[rx], ...rawRecipe, timestamp: moment().unix() };
				saveToLocalStorage(recipes);
				return recipes[rx];
			}
